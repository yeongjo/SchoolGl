...
public:
	...
	// 묶음 갯수 같은거
	GLuint create(vector<vec3>& vertex, GLuint count, GLbyte _mode = color) {
		create2((float*)(&vertex[0]), count, _mode);
		return VAO;
	}

	GLuint create2(float* vertex, GLuint count, GLbyte _mode = color) {
...
float* vec3ArrToFloatArr(std::vector<glm::vec3>& vertexArr, float* arr) {
