...
VO* readObj(const char* file_path) {
	/*FILE* objFile;
	fopen_s(&objFile, file_path, "r");
	if (objFile == NULL){
		print("cant open obj file: %s", file_path);
		return nullptr;
	}
	...
	while (!feof(objFile)) {
		...
		if (line[0] == 'v' && line[1] == '\0') {
			...
			v->vertex.push_back(vertex);
		} else if (line[0] == 'vt' && line[1] == '\0') {
			fscanf(objFile, "%f %f\n", &uv.x, &uv.y);
			v->uv.push_back(uv);
		} else if (line[0] == 'vn' && line[1] == '\0') {
			glm::vec3 normal;
			fscanf(objFile, "%f %f %f\n", &normal.x, &normal.y, &normal.z);
			v->normal.push_back(normal);
		} else if (line[0] == 'f' && line[1] == '\0') {
			int matches = fscanf(objFile, "%d/%d/%d %d/%d/%d %d/%d/%d\n", &vertexIndex[0], &uvIndex[0], &normalIndex[0], &vertexIndex[1], &uvIndex[1], &normalIndex[1], &vertexIndex[2], &uvIndex[2], &normalIndex[2]);
			if (matches != 9) {
				print("File can't be read by our simple parser : ( Try exporting with other options\n");
				return nullptr;
			}
			v->vertexIndices.push_back(vertexIndex[0]);
			v->vertexIndices.push_back(vertexIndex[1]);
			v->vertexIndices.push_back(vertexIndex[2]);
			v->uvIndices.push_back(uvIndex[0]);
			v->uvIndices.push_back(uvIndex[1]);
			v->uvIndices.push_back(uvIndex[2]);
			v->normalIndices.push_back(normalIndex[0]);
			v->normalIndices.push_back(normalIndex[1]);
			v->normalIndices.push_back(normalIndex[2]);
		}
	}*/
